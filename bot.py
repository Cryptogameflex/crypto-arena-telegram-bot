...
async def save_transaction(self, txid: str, user_id: int, amount: float) -> bool:
    ...
    try:
        resp = self.supabase.table("transactions").insert({
            ...
            "amount": amount,
            "verified_at": datetime.now(timezone.utc).isoformat() # Changed to UTC
        }).execute()
...
async def save_subscription(self, user, txid: str):
    ...
    start_date = datetime.now(timezone.utc) # Changed to UTC
    ...
    try:
        response = self.supabase.table("subscriptions").upsert({
            ...
            "created_at": datetime.now(timezone.utc).isoformat() # Changed to UTC
        }).execute()
...
async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if subscription:
        ...
        # Ensure end_date is timezone-aware UTC if it was stored as naive
        if end_date.tzinfo is None:
            end_date = end_date.replace(tzinfo=timezone.utc)

        now_utc = datetime.now(timezone.utc) # Get current time in UTC
        
        # Calculate days left based on calendar dates, ensuring it's not negative
        days_left = max(0, (end_date.date() - now_utc.date()).days)
